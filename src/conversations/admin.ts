...
export async function adminConversation(
  ...
  // ===== LOOP: ro'yxat → amal → yana ro'yxat =====
  while (true) {
    // Ro'yxatni yuklash
    ...
    await ctx.reply(listText, { reply_markup: actionKeyboard(section) });

    const actionCtx = await conversation.waitForCallbackQuery(
      /^(income|expense|firm)_(add|del|back)$/
    );
    ...
    if (action === "back") {
      await actionCtx.editMessageText("⬅️ Orqaga");
      break;
    }
    ...
    if (action === "add") {
      ...
      // loop davom etadi — ro'yxat qayta chiqadi
    } else {
      // O'chirish
      await actionCtx.editMessageText("🗑 O'chirish");

      if (list.length === 0) {
        await ctx.reply("❌ O'chirish uchun element yo'q.");
        continue;
      }
      ...
      delKb.row().text("⬅️ Bekor qilish", "del_cancel");

      await ctx.reply("🗑 O'chirish uchun tanlang:", { reply_markup: delKb });

      const delCtx = await conversation.waitForCallbackQuery(/^del_/);
      await delCtx.answerCallbackQuery();

      if (delCtx.callbackQuery.data === "del_cancel") {
        await delCtx.editMessageText("⬅️ Bekor qilindi");
        continue;
      }
      ...
      if (deleted.success) {
        await delCtx.editMessageText(`✅ O'chirildi: ${deleted.name}`);
      } else {
        await delCtx.editMessageText("❌ O'chirib bo'lmadi. Bog'langan ma'lumotlar bor.");
      }
      // loop davom etadi
    }
  }
